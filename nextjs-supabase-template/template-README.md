...
**IMPORTANT:** Your `next.config.js` (or `next.config.ts`) MUST have `output: 'standalone'` for Docker builds to work:
...
Without this setting, the Docker build will fail because Next.js won't generate the standalone server files.

#### Deployment Steps

1. Create a new Docker Compose deployment in Coolify
2. Point it to your repository containing `docker-compose.production.yml`
3. Coolify automatically generates all environment variables:
   - `SERVICE_PASSWORD_*` - Auto-generated secrets
   - `SERVICE_FQDN_NEXTJSAPP` - Auto-generated domain for your Next.js app
   - `SERVICE_FQDN_SUPABASEKONG` - Auto-generated domain for Supabase API

#### How FQDN Auto-Generation Works

Coolify's magic variables must be **declared** in the environment section to trigger FQDN generation. The format is:

```yaml
- SERVICE_FQDN_<SERVICENAME>_<PORT>
```

For example:
- `SERVICE_FQDN_NEXTJSAPP_3001` - Generates an FQDN and routes traffic to port 3001
- `SERVICE_FQDN_SUPABASEKONG_8000` - Generates an FQDN and routes traffic to port 8000

The compose file already includes these declarations. Coolify will:
1. Generate unique domains (e.g., `nextjsapp-abc123.yourdomain.com`)
2. Populate `SERVICE_FQDN_NEXTJSAPP` and `SERVICE_FQDN_SUPABASEKONG` variables
3. Automatically proxy traffic to the correct ports

### Production Environment Variables

All variables below are **auto-generated by Coolify** when using the production docker-compose:

| Variable | Description |
|----------|-------------|
| `SERVICE_FQDN_NEXTJSAPP` | Auto-generated domain for Next.js app |
| `SERVICE_FQDN_SUPABASEKONG` | Auto-generated domain for Supabase API |
| `SERVICE_SUPABASEANON_KEY` | Anonymous key for client-side requests |
| `SERVICE_SUPABASESERVICE_KEY` | Service role key (server-side only) |
| `SERVICE_PASSWORD_POSTGRES` | Auto-generated database password |
| `SERVICE_PASSWORD_JWT` | Auto-generated JWT secret |
